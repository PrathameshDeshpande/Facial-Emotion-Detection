...
def create_model(input_shape, optimizer='rmsprop', fine_tune=0):
    ...
    top_model = Conv2D(512, kernel_size=(3, 3), activation='relu', padding='same')(top_model)
    top_model = Conv2D(768, kernel_size=(3, 3), activation='relu', padding='same')(top_model)
    top_model = MaxPooling2D(pool_size=(2, 2), strides=(2, 2),padding='same')(top_model)
...
vgg_model = create_model(input_shape,opt, fine_tune=8)
vgg_model.summary()

train_images = np.repeat(train_images, repeats = 3, axis = -1)
valid_images = np.repeat(valid_images, repeats = 3, axis = -1)
test_images = np.repeat(test_images, repeats = 3, axis = -1)
print(train_images.shape)
print(valid_images.shape)

h1 = vgg_model.fit(train_images, train_labels, batch_size=256, epochs=100, verbose=1, validation_data=(valid_images, valid_labels))
